defmodule Dexaggregatex.Market.Server do
  ...
  @spec init(any) ::
          {:ok,
           %MarketServerState{
             market: %Market{pairs: %{}},
             last_update: %LastUpdate{utc_time: nil, pair: nil, timestamp: nil}
           }}
  ...
  """
  @impl true
  @spec handle_call(:get_market, GenServer.from(), MarketServerState.t()) ::
          {:reply, Market.t(), MarketServerState.t()}
  ...
  def handle_call(
        :get_last_update,
        _from,
        %MarketServerState{last_update: %LastUpdate{} = lu} = state
      ) do
    {:reply, lu, state}
  end
  ...
  """
  @impl true
  @spec handle_cast({:update, MarketFetchingPair.t()}, MarketServerState.t()) ::
          {:noreply, MarketServerState.t()}
  def handle_cast(
        {:update, %MarketFetchingPair{} = p},
        %MarketServerState{market: %Market{} = m} = state
      ) do
  ...
  """
  @impl true
  @spec handle_cast({:update, ExchangeMarket.t()}, MarketServerState.t()) ::
          {:noreply, MarketServerState.t()}
  def handle_cast(
        {:update, %ExchangeMarket{} = em},
        %MarketServerState{market: %Market{} = m} = state
      ) do
  ...
  """
  @impl true
  @spec handle_cast({:eat_swept_market, SweepingResult.t()}, MarketServerState.t()) ::
          {:noreply, MarketServerState.t()}
  def handle_cast(
        {:eat_swept_market, %SweepingResult{swept_market: %Market{} = sm, removed_pairs: rp}},
        %MarketServerState{} = state
      ) do
  ...
  defp add_exchange_market(%Market{} = prev_market, %ExchangeMarket{pairs: pairs}) do
    ...
    r =
      Enum.reduce(pairs, initial_return, fn %MarketFetchingPair{} = p, acc ->
  ...
  defp trigger_API_broadcast(%Market{} = updated_market) do
    Task.start(fn ->
      Absinthe.Subscription.publish(Endpoint, updated_market,
        market: "*",
        rebased_market: "*",
        exchanges: "*",
        last_update: "*"
