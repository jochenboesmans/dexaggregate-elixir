defmodule Dexaggregatex.Market.Rebasing.Neighbors do
	...
	def get_neighbors(direction, p_id), do: GenServer.call(__MODULE__, {:get_neighbors, direction, p_id})
	...
	@spec handle_call(call_operation, GenServer.from, State.t) :: {:reply, State.neighbors, State.t}
	@impl true
	def handle_call({:get_neighbors, direction, p_id}, _from, %State{base_neighbors: bn, quote_neighbors: qn} = state) do
		n = case direction do :base -> bn; :quote -> qn end |> Map.get(p_id, [])
		{:reply, n, state}
	end
	...
	def handle_cast({:add_pairs, new_ps, %Market{pairs: pairs} = m},
				%State{base_neighbors: old_bn, quote_neighbors: old_qn} = state) do
			...
			Enum.reduce(old_bn, %{}, fn ({pair_id, bn}, acc1) ->
				...
				new_entry =
					Enum.reduce(new_ps_ids, bn, fn (new_p_id, acc2) ->
						...
						case pim_ba == new_qa, do: (true -> [new_p_id | acc2]; false -> acc2)
					end)
			...
			Enum.reduce(new_ps_ids, interm_bn, fn (new_p_id, acc) ->
				...
				new_bn_entry =
					Enum.reduce(pairs, [], fn ({pair_id, %Pair{quote_address: pim_qa}}, acc) ->
						case pim_qa == new_ba, do: (true -> [pair_id | acc]; false -> acc)
					end)
			...
			Enum.reduce(old_qn, %{}, fn ({pair_id, qn}, acc1) ->
				...
				new_entry =
					...
					Enum.reduce(new_ps_ids, qn, fn (new_p_id, acc2) ->
						%Pair{base_address: new_ba} = pairs[new_p_id]
						case pim_qa == new_ba, do: (true -> [new_p_id | acc2]; false -> acc2)
					end)
				Map.put(acc1, pair_id, new_entry)
			end)
		new_qn =
			...
			Enum.reduce(new_ps_ids, interm_qn, fn (new_p_id, acc) ->
				%Pair{quote_address: new_qa} = pairs[new_p_id]
				new_qn_entry = Enum.reduce(pairs, [], fn ({pair_id, %Pair{base_address: pim_ba}}, acc) ->
					case pim_ba == new_qa, do: (true -> [pair_id | acc]; false -> acc)
				end)
	...
	@impl true
	def handle_cast({:remove_pairs, pairs}, %State{base_neighbors: old_bn, quote_neighbors: old_qn} = state) do
		updated_state =
			Enum.reduce(pairs, %State{base_neighbors: old_bn, quote_neighbors: old_qn} = state,
				fn (%Pair{base_address: ba, quote_address: qa}, %State{base_neighbors: bn_acc, quote_neighbors: qn_acc}) ->
				p_id = pair_id(ba, qa)
				new_bn =
					# Delete deleted pair's entry.
					Map.delete(bn_acc, p_id)
					# Delete pair from base neighbor relations of other pairs.
					|> Enum.reduce(%{}, fn ({n_p_id, bn}, acc) -> Map.put(acc, n_p_id, List.delete(bn, p_id)) end)
				new_qn =
					# Delete deleted pair's entry.
					Map.delete(qn_acc, p_id)
					# Delete pair from quote neighbor relations of other pairs.
					|> Enum.reduce(%{}, fn ({n_p_id, qn}, acc) -> Map.put(acc, n_p_id, List.delete(qn, p_id)) end)
