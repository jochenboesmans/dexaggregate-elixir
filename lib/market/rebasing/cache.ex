defmodule Dexaggregatex.Market.Rebasing.Cache do
  ...
  @spec handle_call({:get, function_call}, GenServer.from, State.t) :: {:reply, get_return, State.t}
  @impl true
  def handle_call({:get, {f, args}}, _from, state) do
    rv = case Map.has_key?(Map.get(state, f), args), do: (true -> {:found, Map.get(state, f)[args]}; false -> {:not_found, nil})
		{:reply, rv, state}
  end
