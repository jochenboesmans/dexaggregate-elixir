defmodule Dexaggregatex.Market.Rebasing do
  ...
  def rebase_pair(%Pair{market_data: pmd} = p, %Market{pairs: pairs} = m, rebase_address, max_depth) do
		rebased_pmd =
	...
	defp try_expand_path([last_p_id | _] = path_to_expand, rebase_address, max_depth, pairs, direction) do
	...
	defp expand_path([last_p_id | _] = path_to_expand, rebase_address, max_depth, pairs, direction) do
		...
		|> Enum.reduce([], fn (n_p_id, paths_acc) ->
			try_expand_path([n_p_id | path_to_expand], rebase_address, max_depth, pairs, direction) ++ paths_acc
		end)
	end
	...
	"""
	@spec deeply_rebase_rate(number, String.t, Market.t, %{base: [String.t], quote: [String.t]}) :: number
	defp deeply_rebase_rate(rate, rebase_address, %Market{pairs: pairs}, %{base: brp, quote: qrp}) do
		r = Enum.reduce(brp, %{combined_volume: 0, volume_weighted_sum: 0}, fn (base_rebase_path, sums) ->
			base_update_sums(sums, base_rebase_path, rate, pairs, rebase_address)
		end)
		%{combined_volume: cv, volume_weighted_sum: vws} = Enum.reduce(qrp, r, fn (quote_rebase_path, sums) ->
	...
	"""
	@spec base_update_sums(%{combined_volume: number, volume_weighted_sum: number}, [String.t], number, map, String.t)
	...
	"""
	@spec quote_update_sums(%{combined_volume: number, volume_weighted_sum: number}, [String.t], number, map, String.t)
				:: %{combined_volume: number, volume_weighted_sum: number}
