defmodule Dexaggregatex.Market.CheapestPaths do
  ...
  def cheapest_path(%RebasedMarket{pairs: pairs} = rm, %Pair{} = initial_p, to_address) do
    ...
    initial_costs =
      Enum.reduce(pairs, %{}, fn {p_id, _p}, acc ->
        v =
          case initial_state.current_p_id == p_id,
            do:
              (
                true -> 0
                false -> :infinity
  ...
  defp try_expand_path(
         %{current_cost: current_cost, current_p_id: current_p_id} = state,
         %RebasedMarket{pairs: pairs} = rm,
         to_address,
         costs,
         visited_pairs
       ) do
  ...
  defp expand_path(
         %{current_cost: current_cost, current_path: [current_p_id | _] = current_path},
         %RebasedMarket{pairs: pairs} = rm,
         to_address,
         costs,
         visited_pairs
    ...
    try_expand_path(
      %{current_cost: current_cost, current_path: [current_p_id | current_path]},
      rm,
      to_address,
      new_costs,
      new_visited_pairs
