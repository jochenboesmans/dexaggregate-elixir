defmodule Dexaggregatex.Market.Client do
	...
	"""
	@spec cheapest_paths(%{from_address: String.t, to_address: String.t, rebase_address: String.t, amount: number})
	...
	defp aggregate_vwsa(%RebasedMarket{pairs: pairs} = _rm) do
		Enum.reduce(pairs, %{}, fn ({k, %MarketPair{quote_address: qa, base_address: ba, market_data: md} = p}, acc) ->
			%{wsa: wsa, tv: tv} =
				Enum.reduce(md, %{wsa: 0, tv: 0}, fn ({_k, emd}, %{wsa: wsa, tv: tv}) ->
					w = ((emd.current_bid + emd.current_ask) / 2) * emd.base_volume
					%{wsa: wsa + w, tv: tv + emd.base_volume} end)
			...
			|> Map.update(ba, %{sum: 0, amount: 0}, fn prev_value ->
				case prev_value do
					...
					_ -> %{sum: safe_div(1, wsa), amount: tv}
				end
			end)
		end)
		...
		|> Enum.reduce(%{}, fn ({k, v}, acc) ->
			case v != 0, do: (true -> Map.put(acc, k, v); false -> acc)
		end)
	end
