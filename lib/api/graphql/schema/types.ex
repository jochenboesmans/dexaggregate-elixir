defmodule Dexaggregatex.API.GraphQL.Schema.Types do
  ...
  object :rebased_market do
    field(:base_address, :string,
      description:
        "The address of the token in which all market data is based. For example: \"0x89d24a6b4ccb1b6faa2625fe562bdd9a23260359\" would indicate that all prices and volumes are denominated in DAI."
    )
  ...
  object :exchange_market_data do
    ...
    field(:current_bid, :float,
      description: "The highest current bid price for the quote token on this exchange."
    )

    field(:current_ask, :float,
      description: "The lowest current ask price for the quote token on this exchange."
    )

    field(:base_volume, :float, description: "This pair's volume on this exchange.")

    field(:timestamp, :integer,
      description: "UNIX timestamp (ms) at which this data was added to the market."
    )
  end
