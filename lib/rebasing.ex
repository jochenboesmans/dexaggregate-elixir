defmodule Rebasing do
	...
	def rebase_market_data(p, rebase_address, market) do
		Enum.reduce(p[:market_data], %{}, fn ({exchange_id, pmd}, acc) ->
			rebased_market_data = %{
				exchange: exchange_id,
				rebased_last_price: deeply_rebase_rate(pmd.last_price, rebase_address, p[:base_address], p[:quote_address], market),
				rebased_current_bid: deeply_rebase_rate(pmd.current_bid, rebase_address, p[:base_address], p[:quote_address], market),
				rebased_current_ask: deeply_rebase_rate(pmd.current_ask, rebase_address, p[:base_address], p[:quote_address], market),
				rebased_base_volume: deeply_rebase_rate(pmd.base_volume, rebase_address, p[:base_address], p[:quote_address], market),
			}
	...
	def combined_volume_across_exchanges(base_address, quote_address, market) do
		pair = market[pair_id(base_address, quote_address)]
		Enum.reduce(pair, 0, fn (pair_on_exchange, sum) -> sum + pair_on_exchange[:market_data][:base_volume] end)
	end
	...
	def volume_weighted_spread_average(base_address, quote_address, market) do
		...
		weighted_sum_of_current_bids = Enum.reduce(pair[:market_data], 0, fn (pmd, sum) ->
			sum + (pmd.base_volume * pmd.current_bid)
		end)
		weighted_sum_of_current_asks = Enum.reduce(pair[:market_data], 0, fn (pmd, sum) ->
			sum + (pmd.base_volume * pmd.current_ask)
		end)
